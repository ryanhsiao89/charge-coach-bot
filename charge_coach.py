...
def call_gemini_with_failover(task_fn, purpose="AI生成"):
    ...
    while True:
        ...
        for _ in range(len(st.session_state.api_keys_list)):
            ...
            try:
                ...
                if is_daily_quota_error(e):
                    mark_key_cooldown(idx, DAILY_QUOTA_COOLDOWN_SECONDS)
                    st.toast(f"{purpose}：第 {idx + 1} 組 Key 今日免費額度已用完，嘗試下一組。", icon="🚫")
        ...
        if last_error and is_daily_quota_error(last_error):
            raise RuntimeError(
                "Gemini 免費層每日額度已用完。請換用不同 Google Cloud project 的 API Key、"
                "開啟 billing，或等每日額度重置後再試。提醒：同一個 Google Cloud project "
                "建立的多支 API Key 會共用 quota。"
            )

        if not waited_once:
            waited_once = True
            wait_seconds = seconds_until_next_key()
            st.warning(f"⏳ {purpose}：API 暫時滿載，等待 {wait_seconds} 秒後重試。")
...
def looks_like_language_drift(text):
    text = str(text)
    ...
    lowered = text.lower()
    if any(phrase in lowered for phrase in drift_phrases):
        return True

    english_words = [
        word for word in re.findall(r"\b[A-Za-z]{4,}\b", text)
        if word.upper() not in {"VIA"}
    ]
    return len(english_words) >= 5


def rewrite_to_traditional_chinese(text):
    prompt = f"""
請把以下回覆完整改寫成自然、溫柔的繁體中文。

規則：
1. 只輸出繁體中文。
2. 不可出現英文、越南文、簡體中文或其他語言。
3. 保留原本溫柔陪伴的意思。
4. 若有括號中的動作描述，也要改成繁體中文。

【待改寫內容】
{text}
""".strip()

    return generate_text_with_failover(
        prompt,
        purpose="繁體中文改寫",
        temperature=0.0,
...
def looks_like_unclear_avoidance_advice(text):
    text = str(text)
    if "後續陪伴時需要避免" in text:
        return True

    avoid_lines = re.findall(r"(?m)^\s*(?:[-•*]\s*)?避免", text)
    return len(avoid_lines) >= 3


def rewrite_to_supportive_clear_subject(text):
    prompt = f"""
請把以下內容改寫成溫柔、正向、主詞清楚的繁體中文。

改寫規則：
1. 不要使用一串「避免...」的負向清單。
2. 若是在提醒後續陪伴方式，主詞請寫清楚，例如「教練可以...」、「下一步陪伴時，可以...」。
3. 把「避免評價」改成「先承接感受，再慢慢理解脈絡」這類可行方向。
4. 保留原本的臨床敏感度與創傷知情精神。
5. 只輸出改寫後內容，不要解釋你做了什麼。
...
def polish_coach_response(text):
    polished = str(text).strip()

    try:
        if looks_like_language_drift(polished):
            polished = rewrite_to_traditional_chinese(polished)

        if looks_like_unclear_avoidance_advice(polished):
            polished = rewrite_to_supportive_clear_subject(polished)
    except Exception:
...
def build_coach_system_prompt(initial_score):
    return f"""
你是「溫充電教練」（Warm Charge Coach）。
你的任務是用創傷知情、優勢觀點與 VIA 性格優勢，陪伴疲憊、壓力大、需要被安頓的學校教師。

【語言鐵則】
1. 所有輸出都必須是繁體中文。
2. 括號中的動作描述也必須是繁體中文。
3. 不可出現英文、越南文、簡體中文或其他語言。
4. 若想描述非語言動作，請使用繁體中文，例如：（我輕輕點頭，表示理解。）
5. VIA 這個縮寫可以保留，其餘內容請使用繁體中文。

【回應語氣與措辭】
1. 優先使用正向、可行、支持性的語句，例如「可以先...」、「下一步可以...」、「教練會陪你...」。
2. 如果需要提醒後續陪伴方向，請明確寫出主詞，例如「教練可以...」、「下一次對話可以...」。
3. 不要輸出一整串「避免...」的清單，因為這會讓使用者感到被要求或被責備。
4. 若需要描述界線，請改成具體支持方式，例如把「避免過度追問」改成「教練可以先停在感受層次，等老師準備好再慢慢探索」。
...
def send_message_safely(user_text):
    def task(_active_key):
        ...
        return polish_coach_response(extract_response_text(resp))
...
3. 已經展現的 VIA 優勢；
4. 曾提過可行或不可行的自我照顧方式；
5. 後續陪伴可以如何支持老師感到更安全、更有選擇。

請不要評分。
請不要寫成督導報告。
請用正向、主詞清楚的語言描述，例如「教練可以...」、「下一次對話可以...」。
請不要輸出「後續陪伴時需要避免踩到的點」或一串「避免...」清單。
請用 350 字以內繁體中文摘要。
...
""".strip()
    ...
    try:
        ...
        st.session_state.memory_summary = polish_coach_response(summary)
...
def get_strengths_readiness():
    user_texts = [
        str(msg.get("content", "")).strip()
        for msg in st.session_state.history
        if msg.get("role") == "user" and str(msg.get("content", "")).strip()
    ]
    total_chars = sum(len(text) for text in user_texts)
    return len(user_texts), total_chars


def strengths_data_is_enough():
    user_count, char_count = get_strengths_readiness()
    return user_count >= MIN_USER_MESSAGES_FOR_STRENGTHS and char_count >= MIN_USER_CHARS_FOR_STRENGTHS


def analyze_strengths():
    st.session_state.strengths_warning = ""

    if not strengths_data_is_enough():
        user_count, char_count = get_strengths_readiness()
        st.session_state.strengths_warning = (
            "這次對話資料還不夠產生穩定的 VIA 雷達圖。"
            f"目前老師回應 {user_count} 則、約 {char_count} 字；"
            "建議至少分享 3 則、合計約 40 字以上，再產生雷達圖會比較可靠。"
        )
        return {}
...
若資料有限，仍請根據可觀察線索給出保守分數，不要拒絕，不要輸出「資料不足」。
...
""".strip()
    ...
    try:
        ...
        st.session_state.strengths_warning = f"VIA 優勢分析暫時失敗：{e}"
        st.warning(st.session_state.strengths_warning)
        return {}
...
if has_api_key:
    ...
    if len(set(st.session_state.api_keys_list)) < len(st.session_state.api_keys_list):
        st.sidebar.warning("偵測到重複的 API Key。相同 Key 不會增加可用額度。")
    elif len(st.session_state.api_keys_list) > 1:
        st.sidebar.caption("提醒：同一個 Google Cloud project 建立的多支 Key 仍會共用每日 quota。")
else:
...
# ------------------------------
    ...
    ready_for_strengths = strengths_data_is_enough()

    if not ready_for_strengths:
        user_count, char_count = get_strengths_readiness()
        st.info(
            "目前仍可生成能量走勢並完成後台紀錄。"
            f"若想要比較穩定的 VIA 雷達圖，建議再回到對話多分享一點：目前老師回應 {user_count} 則、約 {char_count} 字；"
            "建議至少 3 則、合計約 40 字以上。"
        )
